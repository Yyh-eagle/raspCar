...
class KalmanFilter_circle:
    ...
    def _initialize_matrices(self):
        ...
        self.kf.processNoiseCov[0, 0] = 0.01  # x 噪声
        self.kf.processNoiseCov[1, 1] = 0.01  # y 噪声
        self.kf.processNoiseCov[2, 2] = 0.05   # vx 噪声
        self.kf.processNoiseCov[3, 3] = 0.05   # vy 噪声
...
class KalmanFilter_Line:
    """
    功能：卡尔曼滤波器，适用于霍夫直线检测，反馈角度和距离
    输入：当前测量数据 [angle, distance]
    返回值：滤波后的数据 [angle, distance, v_angle, v_distance]
    """
    def __init__(self):
        self.state_size = 4  # 状态向量维度: [angle, distance, v_angle, v_distance]
        self.meas_size = 2   # 测量向量维度: [angle, distance]
        self.control_size = 0
    ...
    def _initialize_matrices(self):
        ...
        self.kf.transitionMatrix = np.eye(self.state_size, dtype=np.float32)
        self.kf.measurementMatrix = np.zeros((self.meas_size, self.state_size), dtype=np.float32)
        self.kf.measurementMatrix[0, 0] = 1  # angle 观测
        self.kf.measurementMatrix[1, 1] = 1  # distance 观测
        self.kf.processNoiseCov = np.eye(self.state_size, dtype=np.float32)
        self.kf.processNoiseCov[0, 0] =1500  # theta 噪声
        self.kf.processNoiseCov[1, 1] = 5000  # distance 噪声
        self.kf.processNoiseCov[2, 2] = 50  # v_theta 噪声
        self.kf.processNoiseCov[3, 3] = 150 # v_distance 噪声

        # 假设theta观测噪声较小，rho噪声较大（霍夫变换对rho更敏感）
        measurement_noise_cov = np.array([0.01, 0.1], dtype=np.float32)
        self.kf.measurementNoiseCov = np.diag(measurement_noise_cov)
    ...
    def KalmenCalculate(self, measurement, dt):
        ...
        angle = self.state[0, 0]
        distance = self.state[1, 0]
        v_angle = self.state[2, 0]
        v_distance = self.state[3, 0]
        #print(angle, distance)
        return [angle, distance]

    def _initialize_tracker(self):
        self.kf.statePost = np.zeros((self.state_size, 1), dtype=np.float32)
        self.kf.statePost[0, 0] = self.meas[0, 0]  # angle
        self.kf.statePost[1, 0] = self.meas[1, 0]  # distance
        self.kf.errorCovPre = np.eye(self.state_size, dtype=np.float32)

    def _update_transition_matrix(self, dt):
        self.kf.transitionMatrix[0, 2] = dt  # angle += v_angle*dt
        self.kf.transitionMatrix[1, 3] = dt  # distance += v_distance*dt
